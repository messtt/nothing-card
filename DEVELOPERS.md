...
Ce dépôt suit l'organisation d'un dépôt de cartes Lovelace moderne : des sources découpées en modules dans `src/`, un
bundle unique livré dans `dist/`, et un banc d'essai qui charge ce bundle sans Home Assistant.
...
Le banc d'essai monte les treize cartes avec un faux objet `hass` : états, `callService`, `callWS`. C'est le moyen le
...
servir telle quelle, sans étape de construction, depuis la branche `main`.
...
Rien d'autre : le bundle, l'entrée du sélecteur de cartes et la bannière console suivent tout seuls.
...
3. création du shadow root et **adoption** de la feuille de style — construite une seule fois par type de carte et
...
`src/var/version.js` porte la version. `npm version patch|minor|major` la recopie depuis `package.json`, reconstruit
`dist/` et met le tout dans le commit.

`dist/nothing-card.js` est **versionné dans git** : HACS et les installations manuelles récupèrent le fichier tel quel,
sans étape de construction. La CI vérifie qu'il correspond bien aux sources (`git diff --exit-code -- dist/`).
